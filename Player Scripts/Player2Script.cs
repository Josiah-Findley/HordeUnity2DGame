...
public class Player2Script : MonoBehaviour
{
    ...
    public int currentGun; //current gun in use 
    ...
    private bool p2_dropcrate = false;
    ...
    {
        //PlayerMovement
        float x = speed * Input.GetAxis("Player2_Horizontal"); //maximum speed * an axis somewhere between +1 and -1. horizontal
        float y = speed * Input.GetAxis("Player2_Vertical"); //maximum speed * an axis somewhere between +1 and -1. vertical
        ...
        //switch gun
        if (Input.GetButtonDown("P2_SwapWeapon") && !atShop)
        {
            switchGun();
        }

        //Consume Health potion
        if (Input.GetButtonDown("P2_ConsumePotion"))
        {
        ...
        if ((Input.GetButtonDown("P2_DropCrate") || (Input.GetAxisRaw("P2_DropCrate") > 0 && !p2_dropcrate)) && isCarryingCrate)
        {
            p2_dropcrate = true;
            ...
            MSMScript.Instance.SpawnCrate(gameObject.transform.position); //spawn crate at location
            droppedCrate = true;
            timeSinceCrateDropped = Time.time;
        }
        if (Input.GetAxisRaw("P2_DropCrate") <= 0)
        {
            p2_dropcrate = false;
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.gameObject.tag == "coin")
        {
            if (!isCoOpMode)
            {
                ...
                MSMScript.Instance.AddOrSubPoints(2, "Enemy");
            }
            else
            {
                collision.gameObject.SetActive(false);
                ...
                CoOpMSMScript.Instance.AddOrSubPoints(2, "Enemy");
            }
        }
        ...
        if (collision.gameObject.tag == "purpleCoin")
        {
            if (!isCoOpMode)
            {
                ...
                MSMScript.Instance.AddOrSubPoints(2, "LaserEnemy");
            }
            else
            {
                collision.gameObject.SetActive(false);
                ...
                CoOpMSMScript.Instance.AddOrSubPoints(2, "LaserEnemy");
            }
        }
        ...
        if (collision.gameObject.tag == "Chest")
        {
            if (!isCoOpMode)
            {
                ...
                MSMScript.Instance.AddOrSubPoints(2, "HVT");
            }
            else
            {
                Destroy(collision.gameObject);
                ...
                CoOpMSMScript.Instance.AddOrSubPoints(2, "HVT");
        ...
        if (collision.gameObject.tag == "HealthOrange")
        {
            Destroy(collision.gameObject);
            PopUpScript.Create(collision.transform.position, CoOpMSMScript.Instance.ENEMY_SCORE, "score");
            CoOpMSMScript.Instance.AddLife("p2");
        }
    ...
    void ForceField()
    {
        if (fieldInactive <= 0)
        {
            ...
            if (Input.GetButtonDown("P2_ForceField"))
            {
                //play our sound
                if (!isCoOpMode)
